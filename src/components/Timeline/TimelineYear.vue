<template>
  <b-row
    class="justify-content-md-center timelineBackground"
    style="
    ...
    <b-col cols="8">
      <div class="d-flex">
        <b-col>
          <button
            ref="prevYearButton"
            class="arrow arrow__prev disabled"
            disabled
            @click="prevYear"
          ></button>
        </b-col>
  ...
  methods: {
    loadYears() {
      ...
      this.$axios(this.urlSiteBase).then((response) => {
        ...
        if (data !== undefined) {
          const itemsSetUrl = data['o:items']['@id'];
            ...
            .then((response) => {
              if (response.data.length > 0) {
                response.data.forEach((year) => {
                  if (
                    typeof year['dcterms:date'] !== 'undefined' &&
                    typeof year['dcterms:description'] !== 'undefined'
                  ) {
                    this.years.push(
    ...
    swipeFnYear() {
      this.$nextTick(() => {
        ...
        this.firstItem = document.querySelector(
    ...
    buttonState() {
      ...
      this.isElementInViewport()
        ? this.setButtonState(this.arrowPrev)
        : this.setButtonState(this.arrowPrev, false);

      this.elementViewPort = this.lastItem;
      this.isElementInViewport()
        ? this.setButtonState(this.arrowNext)
        : this.setButtonState(this.arrowNext, false);
    },
    yearsLoaded() {
