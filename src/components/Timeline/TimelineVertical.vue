        ...
        <div v-for="(itemByYear, index) in itemsOrdered" :key="index" class="items-year">
            ...
            <ul v-for="(month) in itemByYear.months" class="items-month">
                <transition-group name="list">
                    <li v-for="(day, index) in month.days" :key="index" class="items-days">
                        <label class="day">{{ month.monthName }} {{ day.day }}</label>
                        <div class="items" v-for="(item) in day.items" :key="item.index">
    ...
    export default {
        ...
        methods: {
            ...
            loadElementsViewPort() {
                this.$nextTick(() => {
                    let timelinesItem = document.querySelectorAll('.list-item-v');
