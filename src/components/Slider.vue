...
export default {
  ...
  methods: {
    getItemSetClassSlider() {
      this.$axios(
        `${this.$domainOmeka}api/item_sets?resource_class_label=slider&site_id=${this.$idDefauldSite}`
      ).then((itemSet) => {
    ...
    async loadImgSlider(url) {
      ...
      for (const reponseImage of response.data) {
        await this.getImg(reponseImage['o:media'][0]['@id']);
      }
    },
    async getImg(reponseImageUrl) {
      const response = await this.$axios(reponseImageUrl);
